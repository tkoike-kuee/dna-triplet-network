...
def seq_mutate_num(seq, change_num):
    seq_list = list(seq)
    change_index=random.sample(range(len(seq)),change_num)
    for i in change_index:
        seq_list[i] = np.random.choice([base for base in bases if base != seq_list[i]])
...
def random_pairs_in_fixed_num(n,d,fixed):
    targets = [ randseq(d) for _ in range(n) ]

    pairs = np.array(
        [ [ target, seq_mutate_num(target,fixed) ]
